...
character. Existing legacy loot messages remain visible but cannot be assigned
...
Open `/dlt`, then use Older/Newer to browse visits. The view shows looted money,
confirmed sale income, their total, and a timestamped loot and sale history.
...
and other balance changes. Unrecognized money messages remain in the log and
are flagged as missing from the total. Item links and quantities remain visible.
...
- Allow more than 11 sales per visit (off by default; requires confirmation that
...
11 sale attempts per merchant visit by default across all runs and manual/automatic sale
passes. Each stack counts as one sale, regardless of its quantity. At the limit,
check Buyback before closing and reopening the merchant to sell more. Repeated
Sell clicks do not reset the limit. Sales by other addons or manual bag clicks
...
Escape. Its position is saved per character across reloads and logins, and is
